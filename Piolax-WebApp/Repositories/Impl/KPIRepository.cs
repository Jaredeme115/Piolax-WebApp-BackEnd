...
namespace Piolax_WebApp.Repositories.Impl
{
    public class KPIRepository(AppDbContext context) : IKPIRepository
    {
        ...
        public async Task<IEnumerable<KpisDetalle>> ConsultarMTTA(int? idArea = null, int? idMaquina = null, int? anio = null, int? mes = null)
        {
            var query = _context.KpisDetalle
                ...
                .Where(kd => kd.kpiNombre == "MTTA"
                            && kd.KpisMantenimiento.idEmpleado == null  // sólo global
);
            ...
            if (anio.HasValue)
            {
                query = query.Where(kd => kd.KpisMantenimiento.fechaCalculo.Year == anio.Value);
            }
            if (mes.HasValue)
            {
        ...
        public async Task<IEnumerable<KpisDetalle>> ConsultarMTTR(
    int? idArea = null,
    int? idMaquina = null,
    int? idEmpleado = null,
    int? anio = null,
    int? mes = null)
        {
            ...
            // ——> CASO “NINGÚN FILTRO”: ni área, ni máquina, ni técnico
            if (!idArea.HasValue && !idMaquina.HasValue && !idEmpleado.HasValue)
            {
                // Solo el registro global (el que guardaste con idEmpleado = NULL)
                query = query.Where(kd =>
                    kd.KpisMantenimiento.idEmpleado == null);
            }
            else if (idEmpleado.HasValue)
            {
                // Si preguntas por técnico → solo ese técnico
                query = query.Where(kd =>
                    kd.KpisMantenimiento.idEmpleado == idEmpleado.Value);
            }
            else
            {
                // Si preguntas por área y/o máquina (pero NO técnico) → el global de esa combinación
                if (idArea.HasValue)
                    query = query.Where(kd =>
                        kd.KpisMantenimiento.idArea == idArea.Value);
                if (idMaquina.HasValue)
                    query = query.Where(kd =>
                        kd.KpisMantenimiento.idMaquina == idMaquina.Value);
            ...
            if (anio.HasValue)
                query = query.Where(kd =>
                    kd.KpisMantenimiento.fechaCalculo.Year == anio.Value);
            if (mes.HasValue)
                query = query.Where(kd =>
                    kd.KpisMantenimiento.fechaCalculo.Month == mes.Value);
