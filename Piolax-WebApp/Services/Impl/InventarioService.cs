                ...
                using (var package = new ExcelPackage(filePath.OpenReadStream()))
                {
                    ...
                    for (int row = 2; row <= totalRows; row++) // Itera desde la fila 2 (omitir encabezado)
                    {
                        try
                        {
                            ...
                            // 🔍 Validación personalizada para evitar duplicados exactos
                            bool yaExiste = await _repository.ExisteProductoExacto(
                            ...
                            {
                                errores.Add($"Fila {row}: Producto duplicado exacto (nombre, parte, descripción, área, máquina).");
                            }
                        }
                        catch (Exception ex)
                        {
